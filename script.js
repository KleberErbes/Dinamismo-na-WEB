  ...
  async function fetchPosts() {
    document.getElementById('posts-list').innerHTML = `<div class="loader"><div class="spinner"></div><span>Carregando posts…</span></div>`;
    try {
      ...
      document.getElementById('posts-list').innerHTML = `<div class="empty"><div class="empty-icon">⚠️</div><p>Erro ao carregar posts.<br><small>${e.message}</small></p></div>`;
      showToast('Erro ao carregar posts.', 'error');
    }
  }

  async function fetchUsers() {
    document.getElementById('users-list').innerHTML = `<div class="loader"><div class="spinner"></div><span>Carregando usuários…</span></div>`;
    try {
      ...
      document.getElementById('users-list').innerHTML = `<div class="empty"><div class="empty-icon">⚠️</div><p>Erro ao carregar usuários.<br><small>${e.message}</small></p></div>`;
  ...
  function renderPosts(list) {
    ...
    if (!list.length) {
      container.innerHTML = `<div class="empty"><div class="empty-icon">📭</div><p>Nenhum post encontrado.</p></div>`;
      return;
    }
    container.innerHTML = `<div class="grid">${list.map(post => `
      <div class="card" id="post-card-${post.id}">
        <div class="card-id">POST #${post.id} &nbsp; <span class="badge badge-blue">User ${post.userId}</span></div>
  ...
  function renderUsers(list) {
    ...
    if (!list.length) {
      container.innerHTML = `<div class="empty"><div class="empty-icon">👻</div><p>Nenhum usuário encontrado.</p></div>`;
      return;
    }
    container.innerHTML = `<div class="grid">${list.map(u => `
      <div class="card" id="user-card-${u.id}">
        <div class="card-id">USER #${u.id} &nbsp; <span class="badge badge-purple">${escHtml(u.username)}</span></div>
  ...
  function filterPosts() {
    ...
    renderPosts(posts.filter(p => p.title.toLowerCase().includes(q) || p.body.toLowerCase().includes(q)));
  }

  function filterUsers() {
    const q = document.getElementById('search-users').value.toLowerCase();
    renderUsers(users.filter(u => u.name.toLowerCase().includes(q) || u.email.toLowerCase().includes(q) || u.username.toLowerCase().includes(q)));
  }
  ...
  function savePost() {
    ...
    if (!title || !body) { showToast('Preencha título e corpo!', 'error'); return; }

    if (editId) {
      const idx = posts.findIndex(p => p.id == editId);
      if (idx !== -1) { posts[idx].title = title; posts[idx].body = body; posts[idx].userId = userId; }
      showToast('Post atualizado!', 'success');
    } else {
      const newPost = { id: nextPostId++, userId, title, body };
      posts.unshift(newPost);
      document.getElementById('stat-added').textContent = parseInt(document.getElementById('stat-added').textContent) + 1;
  ...
  function saveUser() {
    ...
    if (!name || !username || !email) { showToast('Preencha nome, username e e-mail!', 'error'); return; }

    if (editId) {
      const idx = users.findIndex(u => u.id == editId);
      if (idx !== -1) {
        users[idx].name = name; users[idx].username = username;
        users[idx].email = email; users[idx].company = { name: company };
      }
      showToast('Usuário atualizado!', 'success');
    } else {
      users.unshift({ id: nextUserId++, name, username, email, company: { name: company }, phone: '', website: '', address: {} });
      document.getElementById('stat-users').textContent = users.length;
      populateUserSelect();
  ...
  function populateUserSelect() {
    ...
    sel.innerHTML = users.map(u => `<option value="${u.id}">${u.id} — ${u.name}</option>`).join('');
  }
  ...
  function showToast(msg, type = 'success') {
    ...
    const icon = document.getElementById('toast').querySelector('.ti');
    t.className = type;
